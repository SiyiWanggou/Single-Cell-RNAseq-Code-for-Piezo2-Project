...
#UMAP plot for all 17 clusters (Extended Data Figure 5a.)
...
#Plot Top 3 Marker Genes for each clusters (Extended Data Figure 5b.)
...
#Identify Sox2 and Atoh1 double positive cells. (Extended Data Figure 5c.)
...
#Sox2_Lrrc8a co-expression
Sox2_id <- row.names(subset(fData(HSMM_all),gene_short_name == "Sox2"))
Lrrc8a_id <- row.names(subset(fData(HSMM_all),gene_short_name == "Lrrc8a"))
Sox2_Lrrc8a_exp <- exprs(HSMM_all)[c(Sox2_id,Lrrc8a_id),]
Sox2_Lrrc8a_exp <- as.matrix(Sox2_Lrrc8a_exp)
Sox2_Lrrc8a_exp <- t(Sox2_Lrrc8a_exp)
colnames(Sox2_Lrrc8a_exp) <- c("Sox2","Lrrc8a")
write.table(Sox2_Lrrc8a_exp,"Sox2_Lrrc8a_exp.txt",row.names = TRUE,col.names = TRUE,quote=FALSE,sep='\t')
Sox2_Lrrc8a_exp <- read.delim("Sox2_Lrrc8a_exp.txt",header=TRUE,row.names = 1)
Sox2_Lrrc8a_coexpression <- subset(Sox2_Lrrc8a_exp,Sox2 >= 1 & Lrrc8a >= 1)
pData(HSMM_all)$Sox2_Lrrc8a_coexpression <- rownames(pData(HSMM_all)) %in% rownames(Sox2_Lrrc8a_coexpression)
Cairo(file="Plot_Sox2_Lrrc8a_facet.png",type="png",units="in",bg="white",width=4.9,height=3.5,pointsize=5,dpi=300)
plot_cell_clusters(HSMM_all, 1, 2, color_by = "Sox2_Lrrc8a_coexpression", cell_size = 0.5,show_group_id=FALSE) + 
  facet_wrap(~Group) +
  scale_color_manual(values = c("grey","red")) +
  theme(legend.text=element_text(size=1)) + #set the size of the text
  theme(legend.position="top",
...
#geom_rug()
dev.off()
Sox2_expression <- subset(Sox2_Lrrc8a_exp,Sox2 >= 1)
pData(HSMM_all)$Sox2_expression <- rownames(pData(HSMM_all)) %in% rownames(Sox2_expression)
MS <- row.names(subset(pData(HSMM_all),Group == "MS"))
HSMM_MS <- HSMM_all[,MS]
MS_MB_cells <- row.names(subset(pData(HSMM_MS),Cluster == 1 | Cluster == 2 | Cluster == 5 | Cluster == 9 | 
                                  Cluster == 10 | Cluster == 8 | Cluster == 12 ))
HSMM_MS_MB_Cells <- HSMM_MS[,MS_MB_cells]
table(pData(HSMM_MS_MB_Cells)$Sox2_Lrrc8a_coexpression)
table(pData(HSMM_MS_MB_Cells)$Sox2_expression)
...
marker_genes <- c("Axin2")
Cairo(file="Plot_Axin2_facet.png",type="png",units="in",bg="white",width=4.9,height=3.75,pointsize=5,dpi=300)
...
#jitter-plot of target genes
#Plot jitter plot of Scn3a,Lrrc8a and Axin2.
to_be_tested <- row.names(subset(fData(HSMM_all),gene_short_name %in% c("Axin2")))
Cairo(file="Plot_jitter_Axin2_facet.png",type="png",units="in",bg="white",width=12,height=6,pointsize=5,dpi=300)
cds <- HSMM_all[to_be_tested,]
plot_genes_jitter(cds,
                  grouping = "Group",
                  color_by = "Group",
                  nrow= 3,
                  ncol = NULL,
                  plot_trend = TRUE) +
  facet_wrap(~assigned_cell_type)
dev.off()
#------- End. Charpter 4.--------
...
#Change pre_branch dataframe to branch_1 and branch_2 to regenerate Extended Data Figure 6.
#For pre-branch, Figure 3d uppper-right will be regenerated.
...
summary(a)
b
#Statistics of Pseudotime in over 0.25 course of pre-branch.(Figure 3d lower-right)
...
dev.off()
#Plot Loess Smoothing Fitting (span = 0.75, Figure 4b)
