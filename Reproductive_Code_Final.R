...
Cairo(file="Plot_UMAP_17_cluster_facet.png",type="png",units="in",bg="white",width=6,height=5.4,pointsize=10,dpi=300)
...
Cairo(file="Plot_Marker_genes.png",type="png",units="in",bg="white",width=12,height=6,pointsize=10,dpi=300)
...
Cairo(file="Plot_AUC_of_HEATMAP.png",type="png",units="in",bg="white",width=15,height=4,pointsize=3,dpi=300)
pheatmap(miniAssigMat,annotation_col = annotation_col,annotation_colors = ann_colors,
         ...
         legend = FALSE)
dev.off()
#Plot marker genes of SHH-A,SHH-B and SHH-C in MB cells.Repalce "Sox2" with "Top2a","Cdk1","Mki67","Eif3e","Eef1a1","Boc","Stmn2","Map1b","Tubb2b" one by one.(Figure 3a)
marker_genes <- c("Sox2")
...
#Repeat Charpter 1.2.3 and skip charpter 4
...
AUC_matrix <- getAUC(cells_AUC)[1:11,1:8617]
...
#Repeat Charpter 1.2.3 and skip charpter 4 and charpter 5.
#Install Seurat
...
Gene_index <- row.names(clustering_DEG_genes)[order(clustering_DEG_genes$qval)][1:45]
...
#Rename the clusters with the "Branch1-up", "Branch2-up" and "Pre-branch-up" in clusters_of_DE_genes_target.txt file
...
#GO enrichment ananlysis
c5<-enricher(eg1$SYMBOL,TERM2GENE = c5)
write.table(c5,"up_enrichment_c5_branch2.txt",col.names = TRUE,row.names = FALSE,quote=FALSE,sep="\t")
#Dot plot
...
a<-plot_genes_branched_heatmap(HSMM_SHH_Stem_cells[marker,], branch_point = 1,
                               ...
                               show_rownames = TRUE,return_heatmap = FALSE)
a$ph_res
dev.off()
...
#generate the heatmap of cell cycle normalized matrix heatmap in gitools. Clustering the cells into different clusters in gitools.
#For cells in Seurat identified G1, recluster the cells into two clusters and identify the cells with low G1,S,G2,M AUCell score as G0 cells.
#save the pData(HSMM_SHH_stem_cells), merge the clustering information with pData in excel.
...
#Regenerate G2M Sox2+ cells at Pre-branch (Figure 4c)
G2M <- subset(pData(HSMM_SHH_Stem_cells),Cell_cycle_phase == "G2M")
Cairo(file="Plot_Statistics_Pesudotime_and_G2M.score_G2M_cells_prebranch.png",type="png",units="in",bg="white",width=5,height=3.75,pointsize=10,dpi=300)
ggplot(data=subset(G2M,State == 3),aes(x=Pseudotime,y=G2M.Score,group=Group,col=Group)) +
  stat_smooth(method = 'loess') +
  scale_color_manual(values = c("red","yellow")) +
  labs(title="G2M cells in Pre-branch",x="Pseudotime",y="G2M.score") +
  theme_classic()
dev.off()
#Regenerate G0 Sox2+ cells at Pre-branch (Figure 4c)
G0 <- subset(pData(HSMM_SHH_Stem_cells),Cell_cycle_phase == "G0")
Cairo(file="Plot_Statistics_Pesudotime_and_G1.31g_G0_cells_prebranch.png",type="png",units="in",bg="white",width=5,height=3.75,pointsize=10,dpi=300)
ggplot(data=subset(G0,State == 3),aes(x=Pseudotime,y=G1.31g,group=Group,col=Group)) +
  #geom_point() +
  stat_smooth(method = 'loess') +
  scale_color_manual(values = c("red","yellow")) +
  #facet_wrap(~State) +
  labs(title="G0 cells in Pre-branch",x="Pseudotime",y="G1.score") +
  theme_classic()
dev.off()
#------- End. Charpter 11.---------

#------- Charpter 12. Combined analysis with Pseutotime, cell cycle phase and Pi3k-Akt-mTOR hallmark gene signature in pre-branch.
#Regenerate G1 Sox2+ cells at Pre-branch (Figure 4b)
G1 <- subset(pData(HSMM_SHH_Stem_cells),Cell_cycle_phase == "G1")
Cairo(file="Plot_Statistics_Pesudotime_and_PI3K_AKT_MTOR.104g_G1_cells_prebranch.png",type="png",units="in",bg="white",width=5,height=3.75,pointsize=10,dpi=300)
ggplot(data=subset(G1,State == 3),aes(x=Pseudotime,y=PI3K_AKT_MTOR.104g,group=Group,col=Group)) +
  stat_smooth(method = 'loess') +
  scale_color_manual(values = c("red","yellow")) +
  labs(title="G1 cells in Pre-branch",x="Pseudotime",y="PI3K_AKT_MTOR.104g") +
  theme_classic()
dev.off()
#Regenerate G0 Sox2+ cells at Pre-branch (Figure 4b)
G0 <- subset(pData(HSMM_SHH_Stem_cells),Cell_cycle_phase == "G0")
Cairo(file="Plot_Statistics_Pesudotime_and_PI3K_AKT_MTOR.104g_G0_cells_prebranch.png",type="png",units="in",bg="white",width=5,height=3.75,pointsize=10,dpi=300)
ggplot(data=subset(G0,State == 3),aes(x=Pseudotime,y=PI3K_AKT_MTOR.104g,group=Group,col=Group)) +
  stat_smooth(method = 'loess') +
  scale_color_manual(values = c("red","yellow")) +
  labs(title="G0 cells in Pre-branch",x="Pseudotime",y="PI3K_AKT_MTOR.104g") +
  theme_classic()
dev.off()
